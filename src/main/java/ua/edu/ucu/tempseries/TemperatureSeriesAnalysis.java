...
public class TemperatureSeriesAnalysis {
    ...
    public TemperatureSeriesAnalysis(double[] temperatureSeries) {
        for (double t: temperatureSeries){
            if (t < absZero){
                throw new InputMismatchException("There is no stated temperature less than absolute zero." +
                        " Contact scientists");
            }
        }
    ...
    static public double[] copyArrayWithLen(double[] arr, int newLen, int numElements){
        double[] temp = new double[newLen];
        if (numElements >= 0) System.arraycopy(arr, 0, temp, 0, numElements);
        return temp;
    }
}
